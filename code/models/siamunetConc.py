...
class SiamUnet_conc(nn.Module):
    ...
    def forward(self, s2_1, s2_2, s1_1, s1_2): 
        ...
        input1 = torch.cat((s2_1, s1_1), 1)
        input2 = torch.cat((s2_1, s1_2), 1) ###input 2 also uses s2_1 since we do not use s2_2
        

        """Forward method."""
        # Stage 1
        x11 = self.do11(F.relu(self.bn11(self.conv11(input1))))
        ...
        ####################################################
        # Stage 1
        x11 = self.do11(F.relu(self.bn11(self.conv11(input2))))  
